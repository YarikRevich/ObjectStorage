...
/**
 * Represents exception used when ObjectStorage Cluster full destruction fails.
 */
public class ClusterFullDestructionFailureException extends IOException {
    ...
    public ClusterFullDestructionFailureException(Object... message) {
        super(
                new Formatter()
                        .format("ObjectStorage Cluster full destruction failed: %s", Arrays.stream(message).toArray())
                        .toString());
    }
}
