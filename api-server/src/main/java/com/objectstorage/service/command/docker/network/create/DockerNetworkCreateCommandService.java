package com.objectstorage.service.command.docker.network.create;
    ...
    public DockerNetworkCreateCommandService(String networkName) {
        ...
        this.command = switch (osType) {
            ...
            case UNIX, MAC, ANY -> String.format(
                    "docker network inspect %s >/dev/null 2>&1 || docker network create --driver bridge --subnet=149.156.139.0/28 %s",
                    networkName,
                    networkName);
        };
    }
